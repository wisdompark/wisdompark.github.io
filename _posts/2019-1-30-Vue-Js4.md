...
## ◾참고링크  
1. [Git 에러 CRLF will be replaced by LF (혹은 반대) 핸들링하는 방법](https://blog.jaeyoon.io/2018/01/git-crlf.html) - https://blog.jaeyoon.io/2018/01/git-crlf.html    
## ◾메모장
...
```
리눅스나 유닉스에서는 한줄의 끝이 LF(Line Feed)로 이루어지는 반면, 윈도우에서는 줄 하나가 CR(Carriage Return)와 LF(Line Feed) CRLF로 이루어지기 때문에 어느 쪽을 선택할 지 git에게 혼란을 주는 메시지이다. git에게는 이를 자동변환해 주는 기능이 있으니 그 명령어는 아래와 같다.
```
